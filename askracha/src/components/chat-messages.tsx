...
function ChatMessage({ message }: { message: Message }) {
    ...
    return (
        ...
        <div className={`flex ${isUser ? "justify-end" : "justify-start"} animate-in slide-in-from-bottom-4 duration-500`}>
            <div
                className={`max-w-[calc(100%-4rem)] md:max-w-4xl rounded-lg border p-4 lg:p-6 shadow-sm ${isUser // Reduced mobile padding for p-4
            ...
            >
                <div className="flex items-start gap-4">
                    <div
                        className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${isUser ? "bg-primary-foreground/20" : "bg-primary"
                            }`}
                    >
                        {isUser ? (
                            <User className={`w-4 h-4 ${isUser ? "text-primary-foreground" : "text-primary-foreground"}`} />
                        ) : (
