...
import { X, Sparkles, Database, Brain, FileText, Globe, Loader2, CheckCircle, AlertCircle } from "lucide-react"
            ...
            >
                ...
                <div className="flex items-center justify-between p-6 border-b border-sidebar-border">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-lg font-semibold text-sidebar-foreground">AskRacha</h2>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6">
                    ...
                    <div className="rounded-lg border bg-card p-6">
                            ...
                            {status && (
                                <>
                                    ...
                                    <StatusItem
                                        ...
                                        iconColor={status.index_ready ? "text-green-600" : "text-muted-foreground"}
                        ...
                        {status?.model_info && (
                            <div className="mt-4 pt-4 border-t border-sidebar-border">
                                <p className="text-xs text-sidebar-foreground/70">Model: {status.model_info.llm}</p>
                            </div>
                        )}
                    </div>
                    ...
                    {isInitialized && (!status || !status.documents_loaded) && (
                        <button
                            ...
                            disabled={isLoading}
                            className="w-full inline-flex items-center justify-center rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 bg-black rounded-md shadow-xl cursor-pointer text-primary-foreground hover:bg-primary/90 h-12 px-6 gap-3 text-white"
                        >
                            {isLoading ? (
                                <>
    ...
    return (
        <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
                <Icon className={`h-4 w-4 ${iconColor} ${isLoading ? "animate-spin" : ""}`} />
                <span className="text-sidebar-foreground text-sm font-medium">{value}</span>
            </div>
        </div>
    )
}
